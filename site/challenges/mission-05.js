...
export const MISSION_05_PANELS = Object.freeze([
  ...
  { lang: 'ES', candidates: ['FRÍO', 'FRÍA', 'TRÍO', 'CRÍO', 'BRÍO'], rows: ['TRÍO · ⬛🟩🟩🟩', 'FARO · 🟩⬛🟨🟩'], answer: 'FRÍO', index: 3, extracted: 'I' }
]);
...
export function mountMission05(host, context) {
  ...
  view.content.querySelectorAll('[data-word]').forEach((select) => {
    select.addEventListener('change', () => view.saveData({ words: [...view.content.querySelectorAll('[data-word]')].map((item) => item.value) }));
  });
  (view.saved.stageData?.words ?? []).forEach((value, index) => {
    const select = view.content.querySelector(`[data-word="${index}"]`);
